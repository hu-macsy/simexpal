...
def test_experiments_file_permissions(rel_yml_path):
    ...
    for run in cfg.discover_all_runs():
        ...
        assert try_read_write_and_delete(run.aux_file_path('lock'))
        assert try_read_write_and_delete(run.aux_file_path('run'))
        assert try_read_write_and_delete(run.output_file_path('out'))
        assert try_read_write_and_delete(run.output_file_path('status'))
    ...
    for instance in cfg.all_instances():
        if not instance.is_fileless:
            assert try_read_write_and_delete(instance.fullpath)
